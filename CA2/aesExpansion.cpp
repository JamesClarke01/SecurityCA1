...
uint8_t subByte(uint8_t byte) {
    ...
    uint8_t rightHalf = byte & 0b1111;  //get right 4 bits of byte
    uint8_t leftHalf = (byte >> 4) & 0b1111;  //get left 4 bits of byte

    return sBoxTable[leftHalf][rightHalf];
}
...
uint32_t subWord(uint32_t word) {
    ...
    return (subByte(byte3) << 24) | (subByte(byte2) << 16) | (subByte(byte1) << 8) | subByte(byte0);
...
uint32_t rcon(int j) {
    /*
    uint8_t leftmostByte = pow(2, (j-1));
    
    return leftmostByte << 24;
    */
